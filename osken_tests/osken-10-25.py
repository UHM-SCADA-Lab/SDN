...
class SimpleSwitch13(app_manager.OSKenApp):
    ...
    def _packet_in_handler(self, ev):
        ...
        msg = ev.msg                                        # contains openflow version number, message type (ex: OFPT_PACKET_IN),
        # (length=)length of entire openflow message data field (openflow header plus the entire packet on controlled vlan), 
        # transaction id (idk what it is for), 
        # buffer id (should also be OFP_NO_BUFFER (FF FF FF FF or 4294967295)
        # (total length=) length of entire packet from controlled vlan
        # reason (why the packet got sent to our controller - usually ofpr_no_match)
        # table id (the table that has a low priority flow to this controller (should be 100 in our case))
        # Cookie (should be 0 for our case as we haven't set it up)
        # data of entire packet on controlled vlan (msg.data)
        # in_port, in_physical_port, (I havent seen these be different)
        # vlan ID (but it is (in hex) 100X, where X is the actual vlan id, so for vlan 3 this vlan_id=4099 (4096+3)

        # eliya (format and) print the contents of msg, (except msg.data)

        pkt = packet.Packet(msg.data)                       #this is the entire packet from PRODUCTION VLAN, send to john Parser



        datapath = msg.datapath                             #dathpath id
        ...
        eth = pkt.get_protocols(ethernet.ethernet)[0]       #remove, get from john parser

        added_flow = 'False'                                #need to redo how we check if a flow is added

        hard_timeout = 30                                   #Set hard timeout to 30 seconds
        idle_timeout = 0
        vlan_vid = 3;                                       #remove, get from john parser


        time = datetime.now()                               #maybe remove, decide later
        current_time = time.strftime("%H:%M:%S")            # "

        ethtype_eng = "Unknown"                             #remove, replace with john parser
        
        #remove print stuff, print from seperate program
        dash_line=75*'-'
        short_dash_line = 42*'-'
        ...
        #remove, replace with john parser
        ...
        lights = requests.get('http://10.1.88.5:5000')      # make seperate python file that calls this 
        

        if eth.ethertype == ether_types.ETH_TYPE_LLDP:      # ok for now
            # ignore lldp packet
            return
        ...
        # determine if lights on/off        make seperate python file that returns whether or not a flow can be created
        if (lights.json() == 0):
            light_status = 'Off'
        elif (lights.json() == 1):
            light_status = 'On'
        ...
        # use csv table to find EthType name    - remove
        with open("../Downloads/ieee-802-numbers-1.csv", 'r') as file:
            csvreader = csv.reader(file)
            for row in csvreader:
                if (str(row[1]) == ethtype.zfill(4)):
                    ethtype_eng = row[4]
                    break

        arguments = {'eth_type':ether_types.ETH_TYPE_IP, 'in_port' : in_port, 'eth_src':None}

        match = parser.OFPMatch(**arguments)
        self.add_flow(datapath,1,match,actions,idle_timeout,hard_timeout)
        '''
        # install a flow to avoid packet_in next time - replace all this
        ...
        self.logger.info(log_format_data.format(field_name='Eth Type (Hex)', field_data=ethtype))
        self.logger.info(log_format_data.format(field_name='Eth Type (Eng)', field_data=ethtype_eng))
        ...
        self.logger.info(log_format_data.format(field_name='Physical direction', field_data=status))
        self.logger.info(log_format_data.format(field_name='Lights On?', field_data=light_status))
        #print(tcp_packet)
        #print(icmp_packet)
        
        hexdump.hexdump(msg.data)
        '''

        #self.logger.info(list(msg.data))
